...
### レッスンについて
カウントの減少方式でレッスンのグレードが決まる。  
通常のレッスンは550カウント、ハードレッスンは600カウントになる。  
連続成功で成功率が下がり、失敗で成功率が戻りペナルティカウントが課せられる。

#### 基本計算式
※N=1+(連続成功数-1)x3 | P=Pレッスンレベル(5,10,20レベルでボーナスあり) | B=基礎成功率(80) | G=成功率  
**(B+P)-(Nlog(N))=G**

#### 詳細な仕様
一回の試行に20カウント、失敗時ペナルティに55カウント消費。  
また連続成功時、4カウントずつ時間が増えていく。  
バランス調整用のシミュレーターはlessonSimulatorRe.cppを見ていただければ問題ないと思う。  
グラフにするなら出力部分を調整したgraphMaker版があるのでそちらを活用してほしい。  
一応出力済みのものが\documentにあるので参考になると思う。
...
その状況になると活動中止の旨をアイドルに伝えるコミュが始まります。ライブ会場が用意され、各ランク二つから選べます。  
ラストコンサートのシステムは、これまでのオーディションと同じくランダムです。  
計算式はアケ・箱・SPなどからバランスよくなるように考えて行く予定です。

#### 参考サイト
[アイマスSPWiki](https://www36.atwiki.jp/imas-psp/pages/86.html)
[箱マス等Wiki](https://www23.atwiki.jp/idol-master/pages/125.html)
